...
fn edit_commit_message(commit_message: &str) -> Result<String> {
    ...
    let tmp_file_path = tmp_dir
        .path()
        .join(format!("git-narrator_commit_message_{}.txt", Uuid::new_v4()));
...
mod tests {
    ...
    fn test_edit_commit_message() {
        let tmp_dir = Builder::new()
            ...
            .tempdir()
            .unwrap();
        let tmp_file_path = tmp_dir.path().join("git-narrator_commit_message.txt");
        env::set_current_dir(&tmp_dir).unwrap();
    ...
    async fn test_handle_config_command_get() {
        let tmp_dir = Builder::new()
            ...
            .tempdir()
            .unwrap();
        let config_dir = tmp_dir.path().join(".config").join("git-narrator");
